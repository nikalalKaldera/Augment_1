...
parser.add_argument('-d', '--datapath', default='/home/ntkkh958/My_work/Codes/CTR_NEW_26/CTR-GCN/data/ntu120/NTU120_CSub.npz',
                    help='location of dataset npz file')
parser.add_argument('-o', '--outdatapath', default='./NTU120_CSub.npz',
                    help='location of dataset npz file')
...
def fft_augment(data, bone_pairs, num_joint):
    ...
    for index in tqdm(range(data.shape[0])):  # data.shape[0]
        ...
        if (data_numpy[1] == np.zeros((C, T, V), np.float32)).all():
            ...
            for c in range(1, 4):   #Compute the absolute values of the spectral features.
                ...
                amplitude_component[:, c - 1, :, :] = np.abs(spatial_temporal_spectral['ax_{:d}'.format(c)])
            ...
            for m in range(2):
                ...
                # temp_phase.append(spatial_temporal_phase)
                # temp_amp.append(spatial_temporal_amp)
    ...
    return amplitude, phase, max_joint, min_joint


if __name__ == '__main__':
    arg = parser.parse_args()
    data_train, label_train = load_data(arg.datapath)  # N C T V M
    print("start Cropping train dataset to average valid frame length")
    ...
    data_normalised = skeleton_normalization(data_trans)
    np_seq = np.array(data_normalised).transpose(0, 4, 2, 3, 1)  # N, C, T, V, M
    print('start augmentation')
    amplitude, phase, max_joint, min_joint = fft_augment(np_seq, ntu_skeleton_bone_pairs, arg.joints)
    print('Create file')
    np.savez(arg.outdatapath, JFT_amplitude=amplitude, JFT_phase=phase, JFT_min_joint=min_joint, JFT_max_joint=max_joint)
